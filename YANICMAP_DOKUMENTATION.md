...
Dieses Teilprojekt liefert die Datengrundlage fuer eine visualisierte Karte aller Freifunk-Knoten in Hattingen:
...
1. Datenerfassung
   - Yanic fragt Respondd zyklisch per Multicast ab (`ff05::2:1001` und fuer Altgeraete `ff02::2:1001`).
   - Erfassung laeuft ueber Interface `bat0`.

2. Datenpersistenz und Ausgabe
   - Yanic haelt internen Node-Cache in `state.json`.
...
3. Visualisierung
   - Meshviewer liest diese Dateien ueber HTTP vom Webspace.
...
## Empfohlene naechste Schritte fuer die Projektfortfuehrung

1. Upstream-Quelle fuer Yanic auf Codeberg umstellen.
...
4. JSON-Auslieferung absichern:
   - Wenn moeglich SFTP oder HTTPS API statt FTPS mit `--insecure`.
5. Betriebsdokumentation konsolidieren:
   - Ein gemeinsamer "Single Source of Truth" fuer Deploy, Betrieb, Monitoring und Rollback.
...
3. Output pruefen (`docker/data/*.json`, `docker/data/*.geojson`).
4. Upload-Job per Cron ausfuehren.
5. Meshviewer auf gueltige Daten-URLs konfigurieren (`config.json` im Meshviewer-Webroot).
...
- Webhosting: statische Auslieferung der JSON-Dateien fuer Meshviewer
